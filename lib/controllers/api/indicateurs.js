...
exports.find = function(req, res) {
  ...
  var promise = Q.all([
    ...
    _.each(tasks, function(rowdata, index) {  // pour chaque enregistrement
	...
	dbIndicateurs.find(idSelector, function (err, docs) {
		_.each(docs, function(rowdata, index) {  // pour chaque enregistrement
			...
			switch(rowdata.action) {
				...
				rowdata.mesureRef = [];
					...
					var even = _.filter(rowdata.mesures, function(mesure){ 
						if (typeof checksType !== "undefined") {
							...
							_.each(checksType, function(check) {
					         if (mesure[rowdata.type] == check) { 
					         	blnFind = true;
   								rowdata.mesureVal.push({value:mesure[rowdata.type],date:mesure.date});
					...
					var evenRef = _.filter(rowdata.mesures, function(mesure){ 
						if (typeof checksRefType !== "undefined") {                                
							...
							_.each(checksRefType, function(check) {
					         if (mesure[rowdata.reftype] == check) { 
					         		blnFind = true;
					         		rowdata.mesureRef.push({value:mesure[rowdata.type],date:mesure.date});
							 } ; // avec indexOf pour le like
					     });
							return blnFind;
						} else {
					...
					rowdata.mesureValCal = rowdata.mesureVal.length;
					rowdata.mesureRefCal = rowdata.mesureRef.length;
			...
			rowdata.percentObjectif = (rowdata.mesureValCal / rowdata.mesureRefCal) * 100 +'%';
		});
		res.send(docs);
