(function ($) {
   ...
   */
   function DataView(options) {
    ...
    function getMyLength(level,colonne) {
      var FilterTotal=0;
      var Filternb=0;
      for (var i = 0; i < rows.length; i++) {
       if (level == 0) {
        if (level==rows[i].level ) {
         FilterTotal += (rows[i].count);
       }
     } else if (rows[i].__groupTotals) {
      if (colonne == "alerte") {
       FilterTotal += rows[i].sum[colonne];
     }
     else
     {
       FilterTotal += rows[i].avg[colonne];
       Filternb++;
     }
   }
 }

 return Filternb > 0 ? Math.round(FilterTotal/Filternb) : FilterTotal;
}
