...
.controller('MainCtrl', function($rootScope, $scope, $http, $modal, $dialogs){
...
$scope.colorFunction = function() {
	...
	$scope.LoadWidgets = function(){
		$http.get('/REST/indicateurs').success(function (data) {
			...
			//Join json
			// _.each(_.keys($scope.widgets), function(key) {
			// 	var even = _.where($rootScope.contextes, {id: $scope.widgets[key].refContexte});
			// 	$scope.widgets[key].Contexte = even[0];
			// 	even = _.where($rootScope.activities, {id: $scope.widgets[key].refActivity});
			// 	$scope.widgets[key].Activity = even[0];

			// 	$http.get('/REST/mesures/byIndicateur/'+$scope.widgets[key].refContexte+'/'+$scope.widgets[key].refActivity).success(function (mesures) {
			// 		$scope.widgets[key].Mesures = mesures;
			// 		console.log(mesures);
			// 	});
			// });
			});
	...
	$scope.openIndicateur = function (Indicateur) {
		...
		var modalInstance = $modal.open({
			templateUrl: 'myIndicateurContent.html',
